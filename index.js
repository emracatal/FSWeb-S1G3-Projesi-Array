...
Orijinal tatların bulunduğu toplam 25 aromanın olduğu bir dizimiz var (yukarıya bakın). 
Bu görevde, bu dizi'ye veri yazma ve bu diziden veri okuma işlemleri yapacağız.
...
console.log("kopyalama sorusu", kopyala(orijinalTatlar));

/* Görev 2: ARRAY.LENGTH
...
function sonCesitiKaldir(dizi, cesit) {
  dizi.pop(cesit);
  return dizi;
}
...
Aşağıdakileri yapmak için aşağıdaki indekstekiCesitiGetir işlevini kullanın:
   ...
   Örneğin: indekstekiCesitiGetir(orijinalTatlar, 2) çalıştırılmasıyla, 
   Kakule'in başarıyla eklendiği varsayarsak sonuç "Ceviz" olucaktır.
*/
...
/* Görev 6: ARRAY.SPLICE & text.INDEXOF
...
function ismeGoreCesitCikar(dizi, cikar) {
  let sirasi = dizi.indexOf(cikar);
  dizi.splice(sirasi, 1);
  return dizi;
}
console.log(ismeGoreCesitCikar(orijinalTatlar, "Tarçın"));

/* Görev 7: ARRAY.includes
...
/* ALIŞTIRMA */
/* ALIŞTIRMA 1: ORTALAMA KELİME SAYISI?
Bir dizideki ortalama kelime sayısını döndüren bir fonksiyon yazın. 
Bu işlevi orijinalTatlar dizisinde test etmelisiniz, ancak bunu herhangi bir dizi için de kullanabilmelisiniz.
...
function ortalamaKelimeSayisi(dizi) {
  let toplam = 0;
  for (let i = 0; i < dizi.length; i++) {
    toplam = toplam + dizi[i].split(" ").length;
  }
...
Toplam 25 lezzet aromasını orijinalTatlar, yeniTatlar, mevsimlikTatlar ve bolgeselTatlar'dan rastgele seçecek 
ve bunu rastgeleTatlar adlı bir dizide saklayan bir fonksiyon yazın.
...
Aşağıdakileri yapmak için rastgeleTatlar işlevini ve yeni dizileri kullanın:
  ...
  Örneğin: rastgeleTatlar(orijinalTatlar, yeniTatlar, mevsimlikTatlar, bolgeselTatlar) çalıştırıldığında 
...
function rastgeleTatlar(dizi1, dizi2, dizi3, dizi4) {
  const tumdiziler = [...dizi1, ...dizi2, ...dizi3, ...dizi4];
  const rastgeledizi = [];
  for (let i = 0; i < 25; i++) {
    const index = Math.floor(Math.random() * 25);
    rastgeledizi.push(tumdiziler[index]);
  }
  return rastgeledizi;
}
console.log(
